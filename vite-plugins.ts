...
function copyFile(source: string, dest: string, description?: string): boolean {
  try {
    ...
    console.log(
      `✅ 已复制: ${fileName}${description ? ` (${description})` : ""}`
    );
    return true;
  } catch (error) {
...
function modifyManifestForDev(sourcePath: string, destPath: string): void {
  try {
    ...
    // 修改 main 字段为开发服务器地址
    manifest.main = "http://localhost:3020/";
    writeFileSync(destPath, JSON.stringify(manifest, null, 2), "utf-8");
    console.log(
      "🔧 开发模式：已将 manifest.json 的 main 设置为 http://localhost:3020/"
    );
  } catch (error) {
 ...
 */
export function devWatchPlugin(options: DevWatchPluginOptions = {}): Plugin {
  const {
...
export function copyFolderPlugin(
  ...
  const { folderPath = resolve(__dirname, "./dist") } = options;
